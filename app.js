...
createTable('stateProductTable', ['State / Product', 'Leads', 'Est. Spend', 'Net Revenue', 'Net ROAS'],
  stateProduct.map(d => [d.segment, d.leads.toLocaleString(), moneyK(d.spend), moneyK(d.revenue), roas(d.roas)]));
...
function renderStateChart(type = 'spend') {
  ...
  const data = [...stateProduct]
    ...
    .map(d => ({ label: d.segment, value: d[config.key], display: config.formatter(d[config.key]), color: d.color }));
...
const quadrants = [
  ...
  { id: 'optimize', title: 'Optimize before scaling', color: COLORS.orange, items: ['NJ Doors: largest segment | 1.96x ROAS', 'RI Doors: 56 leads | 2.23x ROAS', 'Improve routing + close discipline before adding budget'] },
